...
{% post_link java/java-14-Timer Timer%}，该定时器实现原理是通过单个守护线程轮询监听优先级队列中任务方式，根据任务的执行时机决定是否执行任务。
...
### 4.2 {% post_link Zookeeper/ZK-3-8源码-05-FileTxnSnaplog FileTxnSnaplog %}
...
###### 4.4.2.2 {% post_link Zookeeper/ZK-3-8源码-02-CnxnChannelHandler CnxnChannelHandler %}
...
{% post_link Zookeeper/ZK-3-8源码-05-FileTxnSnaplog 借助FileTxnSnapshot文件管理器将已经持久化的文件加载到内存中。 %}
...
#### 4.5.2 {% post_link Zookeeper/ZK-3-8源码-03-SessionTrackerImpl SessionTracker %}
...
#### 4.5.3 {% post_link Zookeeper/ZK-3-8源码-06-RequestProcessor RequestProcessor %}
...
#### 4.5.4 {% post_link Zookeeper/ZK-3-8源码-04-RequestThrottler RequestThrottler %}
